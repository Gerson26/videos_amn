...
html;
        ...
        View::set('tabla', $this->getAllComprobantesPagoById($_SESSION['id_registrado']));
    ...
    public function getAllComprobantesPagoById($id_user)
    {
        ...
        foreach (ComprobantePagoDao::getAllComprobantes($id_user) as $key => $value) {
            ...
            if ($value['tipo_pago'] == "Efectivo" || $value['tipo_pago'] == "Transferencia" || $value['tipo_pago'] == "") {
                ...
                foreach (ComprobantePagoDao::getAllComprobantesStatusCero($id_user, $value['clave']) as $key => $value) {
                    ...
                    if($value['es_congreso'] == '1'){
                        ...
                        if($value['porcentaje_beca'] < 100){
    
                        $valor = ($value['porcentaje_beca'] * $value['costo']) / 100;
                ...
                $reimprimir_ticket = "<form method='POST'  action='https://www.paypal.com/es/cgi-bin/webscr' data-form-paypal=" . $value['id_pendiente_pago'] . ">
                    <input type='hidden' name='business' value='acameci@amc.org.mx'> 
                    ...
                    <input type='hidden' name='return' value='https://congreso.amc.org.mx/OrdenPago/PagoExistoso/?productos=$array_pro'> 
            ...
            if ($value['tipo_pago'] == "Transferencia" && empty($value['url_archivo'])) {
                ...
                $button_comprobante = '<a href="/comprobantesPago/'.$_SESSION['id_registrado'].'/' . $value["url_archivo"] . '" class="btn bg-pink btn-icon-only morado-musa-text text-center"  data-bs-toggle="tooltip" data-bs-placement="top" data-bs-original-title="Ver mi comprobante" target="_blank"><i class="fas fa-print"> </i></a>';
            }
...
html;
            ...
            foreach (ComprobantePagoDao::getAllComprobantesbyClave($id_user, $value['clave']) as $key => $value2) {
                ...
                if($value2['es_congreso'] == '1'){
                    ...
                    if($value2['porcentaje_beca'] < 100){

                    $valor = ($value2['porcentaje_beca'] * $value2['costo']) / 100;
                    $precio = $value2['costo'] - $valor;
                ...
                if ($value2['status'] == 0 && $value2['url_archivo'] == '') {
                    $icon_status = '<i class="fa fad fa-hourglass" style="color: #F2B500;"></i>';
                    $status = '<span class="badge badge-wait">Realiza tu Pago</span>';
                }else if ($value2['status'] == 0) {
                ...
                } else if($value2['status'] == 2 && $value2['url_archivo'] == ''){
...
html;
    ...
    public function ticketImp($clave)
    {
        ...
        // if(strlen(intval($clave)) == 1){
        //     $f= 'P-000';
        // }elseif(strlen(intval($clave)) == 2){
        //     $f= 'P-00';
        // }elseif(strlen(intval($clave)) == 3){
        //     $f= 'P-0';
        // }
        // elseif(strlen(intval($clave)) == 4){
        //     $f= 'P-';
        // } 
        ...
        $pdf->SetTextColor(0, 0, 0);
        $pdf->Multicell(100, 10, $clave.'-'.$datos_user['id_registrado'], 0, 'C');
    ...
    public function uploadComprobante()
    {
        ...
        $nombre_fichero = 'comprobantesPago/'.$_SESSION['id_registrado'];


        if (!file_exists($nombre_fichero)) {
            mkdir('comprobantesPago/'.$_SESSION['id_registrado'], 0777, true);
        } 

        if ($file['name'] != "") {

            if (move_uploaded_file($file["tmp_name"], "comprobantesPago/".$_SESSION['id_registrado']."/" . $name_archivo)) {
