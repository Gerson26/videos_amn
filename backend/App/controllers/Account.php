...
html;
        ...
        foreach(RegisterDao::getAllEspecialidades() as $key => $value){
            $selectedEspecialidad = ($value['id_especialidad'] == $userData['id_especialidad']) ? 'selected' : '';
...
html;
    ...
    public function ActualizarFiscal(){
        ...
        $documento = new \stdClass();
          ...
          if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            $documento->_razon_social = $razon_social;
            $documento->_rfc = $rfc;
            $documento->_cp_fiscal = $cp_fiscal;
            $documento->_email_fac = $email_fac;
            $documento->_direccion_fiscal = $direccion_fiscal;
    ...
    public function deleteFile($id_registro){
        $regitser = RegisterDao::getUser($id_registro)[0];
