...
sql;
    ...
    public static function getCongresoUno(){
      $mysqli = Database::getInstance();
      $query=<<<sql
        SELECT pg.*
        FROM programa pg
        WHERE pg.id_producto = 2
        ORDER BY pg.hora_inicio ASC
sql;
      return $mysqli->queryAll($query);
    }

    public static function getCongresoDos(){
      $mysqli = Database::getInstance();
      $query=<<<sql
        SELECT pg.*
        FROM programa pg
        WHERE pg.id_producto = 3
        ORDER BY pg.hora_inicio ASC
sql;
      return $mysqli->queryAll($query);
    }

    public static function getCongresoTres(){
      $mysqli = Database::getInstance();
      $query=<<<sql
        SELECT pg.*
        FROM programa pg
        WHERE pg.id_producto = 4
        ORDER BY pg.hora_inicio ASC
sql;
      return $mysqli->queryAll($query);
    }
...
sql;
  ...
  public static function getProgreso($id,$num_curso){
    ...
    WHERE id_video = $num_curso AND id_registrado = $id
sql;
