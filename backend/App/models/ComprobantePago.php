...
sql;
    ...
    public static function getAllComprobantes($id){
      $mysqli = Database::getInstance();
      $query=<<<sql
      SELECT pro.id_producto,pro.nombre,pro.precio_publico,pro.precio_socio,pro.tipo_moneda,pro.caratula,pro.es_curso,pro.es_servicio,pro.es_congreso,pp.id_pendiente_pago,pp.status,pp.tipo_pago,pp.url_archivo,pp.clave,pp.monto,c.costo,ua.porcentaje_beca
      FROM productos pro
      INNER JOIN pendiente_pago pp ON (pro.id_producto = pp.id_producto)
      INNER JOIN registrados ua ON(ua.id_registrado = pp.id_registrado)
      INNER JOIN categorias c ON (c.id_categoria = ua.id_categoria)
      WHERE pp.id_registrado = $id  GROUP BY pp.clave
sql;
      return $mysqli->queryAll($query);
    }
    ...
    public static function getAllComprobantesStatusCero($id,$clave){
      ...
      INNER JOIN registrados ua ON(ua.id_registrado = pp.id_registrado)
      INNER JOIN categorias c ON (c.id_categoria = ua.id_categoria)
      WHERE pp.id_registrado = $id AND pp.status = 0 AND pp.clave = '$clave' 
sql;
      return $mysqli->queryAll($query);
    }
    ...
    public static function getAllComprobantesbyClave($id,$clave){
      $mysqli = Database::getInstance();
      $query=<<<sql
      SELECT pro.nombre,pro.precio_publico,pro.precio_socio,pro.tipo_moneda,pro.caratula,pro.es_curso,pro.es_servicio,pro.es_congreso,ua.clave_socio,pp.id_pendiente_pago,pp.status,pp.tipo_pago,pp.url_archivo,pp.clave, pp.monto,pp.comprado_en,c.costo,ua.porcentaje_beca
      FROM productos pro
      INNER JOIN pendiente_pago pp ON (pro.id_producto = pp.id_producto)
      INNER JOIN registrados ua ON(ua.id_registrado = pp.id_registrado)
      INNER JOIN categorias c ON (c.id_categoria = ua.id_categoria)
      WHERE pp.id_registrado = $id and pp.clave = '$clave'
sql;
      return $mysqli->queryAll($query);
    }
